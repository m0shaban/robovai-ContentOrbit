...
def setup_google_credentials():
    ...
    if creds_json:
        logger.info(
            "🔐 Found GOOGLE_CREDENTIALS_JSON in env. Restoring service_account.json..."
        )
        try:
            # Validate JSON
            ...
            logger.error(
                "❌ Failed to decode GOOGLE_CREDENTIALS_JSON. Is it valid JSON?"
            )
        except Exception as e:
        ...
        if os.path.exists(render_secret_path):
            ...
            # Let's try to copy.
            try:
                with open(render_secret_path, "r") as secret, open(
                    creds_path, "w"
                ) as dest:
        ...
        logger.warning(
            "⚠️ No service_account.json found and no GOOGLE_CREDENTIALS_JSON env var (or Secret File) set. Sheets Control Room will be disabled."
        )
